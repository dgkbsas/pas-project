...
// Base schema for policy object structure
const basePolicySchema = z.object({
	...
	accessories: z.string().max(2000, 'Los accesorios son muy largos').optional().nullable(),
	premium: z.number().nonnegative('El premio debe ser positivo').optional().nullable(),
	endorsement: z.string().max(2000, 'El endoso es muy largo').optional().nullable(),
	observations: z.string().max(1000, 'Las observaciones son muy largas').optional().nullable(),
	active: z.boolean().default(true),
	created_by: z.string().uuid().optional()
});

// Schema for creating policies with date validation
export const policySchema = basePolicySchema.refine(
	(data) => {
...
// Schema for updating policies - all fields optional except validation rules still apply
export const updatePolicySchema = basePolicySchema.omit({ client_id: true, created_by: true }).partial().refine(
	(data) => {
		if (data.start_date && data.expiry_date) {
			return new Date(data.expiry_date) > new Date(data.start_date);
		}
...
// Labels para los tipos de pólizas
