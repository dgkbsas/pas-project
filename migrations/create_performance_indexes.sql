-- Performance Indexes for PASManager
...
-- Enable pg_trgm extension for full-text search (GIN indexes)
...
CREATE INDEX IF NOT EXISTS idx_policies_policy_number_gin 
...
-- Full-text search indexes for clients
CREATE INDEX IF NOT EXISTS idx_clients_first_name_gin 
ON clients USING gin(first_name gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_clients_last_name_gin 
ON clients USING gin(last_name gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_clients_email_gin 
ON clients USING gin(email_primary gin_trgm_ops);
